...
function appendAuthorLabel(container, data) {
  ...
  if (data.authorRole === "admin") {
    ...
    badge.className = "admin-author-icon";
    badge.title = "인증된 관리자 작성";
    badge.setAttribute("aria-label", "인증된 관리자 작성");
    badge.textContent = "✦";
  ...
  const unsubscribe = db.collection("posts").doc(postId).collection("comments")
    ...
    .onSnapshot((snapshot) => {
      ...
      snapshot.forEach((commentDoc) => {
        ...
        meta.className = "comment-meta author-label";
        appendAuthorLabel(meta, comment);
        meta.append(` · ${formatDate(comment.createdAt)}${comment.updatedAt ? " · 수정됨" : ""}`);
...
function renderPostRow(post, index, tableBody) {
  ...
  authorCell.className = "col-author author-label";
  appendAuthorLabel(authorCell, data);
...
firebase.auth().onAuthStateChanged((user) => {
  ...
  if (loginBtn) {
    ...
    loginBtn.textContent = user ? "계정 전환" : "로그인";
    loginBtn.setAttribute("aria-label", user ? "다른 계정으로 로그인" : "Google 계정으로 로그인");
  }
  if (logoutBtn) {
